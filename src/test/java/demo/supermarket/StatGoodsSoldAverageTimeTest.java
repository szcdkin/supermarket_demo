...
public class StatGoodsSoldAverageTimeTest {


    private Supermarket supermarket;

    private long startSell;
    ...
    public void before() {
        ...
        startSell = System.currentTimeMillis();
    ...
    public void test() {
        ...
        for (CustomerOrder order : this.supermarket.getOrders()) {
            if (order.getCreated() > 0 && order.getFinished() > 0 && order.getFinished() > startSell) {
                totalSold += (order.getFinished() - startSell);
                totalCount++;
            }
        }
        long averageTime = totalSold / (totalCount == 0 ? 1 : totalCount);

        Assert.assertTrue(averageTime == this.supermarket.statGoodsSoldAverageTime(startSell));
    }
